...
final class Endpoints {
	...
	public function register() {
		...
		add_action( 'heera_agent_discovery_cache_flushed', array( $this, 'schedule_warm' ) );
		add_action( 'heera_agent_discovery_warm_llms_full', array( $this, 'warm_llms_full' ) );
	}
	...
	public function schedule_warm() {
		...
		if ( function_exists( 'wp_next_scheduled' ) && function_exists( 'wp_schedule_single_event' )
			&& ! wp_next_scheduled( 'heera_agent_discovery_warm_llms_full' ) ) {
			wp_schedule_single_event( time() + 30, 'heera_agent_discovery_warm_llms_full' );
		}
	}
	...
	public function route() {
		...
		// only while Heera Discovery actually owns the sitemap (core/SEO absent), so we
		// never shadow another plugin's file.
		if ( 0 === strpos( $path, '/heera-agent-discovery-sitemap' ) && '.xml' === substr( $path, -4 ) ) {
			if ( 'heera-agent-discovery' === Sitemap::detect()['source'] ) {
	...
	/**
	 * Whether Heera Discovery should STAND DOWN for an agent-readiness surface, so another
	 ...
	 *
	 *     add_filter( 'heera_agent_discovery_yield_surface', function ( $yield, $surface ) {
	 ...
	 * @return bool True if Heera Discovery must not handle this surface.
	 */
	private function yields( $surface ) {
		/**
		 * Cede an agent-readiness surface to another producer.
		 *
		 * @param bool   $yield   Whether Heera Discovery should stand down. Default false.
		 * @param string $surface Surface key (llms_txt|llms_full|markdown|link_headers|robots).
		 */
		return (bool) apply_filters( 'heera_agent_discovery_yield_surface', false, $surface );
	}
	...
	public function llms_full_txt() {
		...
		$item_cap = (int) apply_filters( 'heera_agent_discovery_llms_full_item_max_bytes', min( 256 * 1024, max( 32 * 1024, intdiv( $budget, 4 ) ) ) );
	...
	private function topics() {
		$exclude = apply_filters( 'heera_agent_discovery_topic_exclude', array( 'uncategorized' ) );
		...
		foreach ( $cats as $cat ) {
			...
			if ( '' === $desc ) {
				...
				$desc = sprintf( _n( '%s article', '%s articles', $cat->count, 'heera-agent-discovery' ), number_format_i18n( $cat->count ) );
			}
